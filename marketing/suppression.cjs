...
const LOG_FILES = [
  join(MARKETING_DIR, "mcp-partner-log.json"),
  join(MARKETING_DIR, "data", "drip-log.json"),
  join(MARKETING_DIR, "..", "..", "Desktop", "Email Outreach", "day3-send-log.json"),
  join(MARKETING_DIR, "..", "..", "Desktop", "Email Outreach", "day7-send-log.json"),
  join(MARKETING_DIR, "..", "..", "Desktop", "Email Outreach", "mnemopay-b2b-log.json"),
  join(MARKETING_DIR, "..", "..", "Desktop", "Email Outreach", "resend-fix-45-log.json"),
];

const SUPPRESSION_FILE = join(MARKETING_DIR, "data", "suppression.json");

function getRecentlySent(withinDays = 3) {
...
function filterEmails(emails, opts = {}) {
  const { dedupeWindowDays = 3, verbose = true } = opts;
  ...
  for (const email of emails) {
    const addr = (email.to || email.email || "").toLowerCase();
...
module.exports = { filterEmails, getRecentlySent, getSuppressed };
