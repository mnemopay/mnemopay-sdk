...
describe('MnemoPay SDK Full Suite', () => {
  ...
  describe('MemoryStore', () => {
    it('should store, recall, and query by vector similarity', async () => {
      ...
      const mem = await sdk.memory.retain(content, {
        source: 'observation',
        sessionId: 'session-1',
        tags: ['secret'],
        importance: 0.8,
      ...
      // Use identical text for recall because the placeholder embedding is hash-based
  ...
  describe('WalletEngine', () => {
    ...
    it('should handle escrow flow', async () => {
      const amount = 1000n;
      const escrow = await sdk.wallet.createEscrow(recipientId, amount, [
        { type: 'spatial_proof', params: { h3Tile: 'test-tile' } }
      ]);
  ...
  describe('SpatialProver', () => {
    it('should generate and verify a spatial proof', async () => {
      ...
      const sensorReadings = { wifi: ['ap-1', 'ap-2'] };
      const deviceId = 'device-abc-123';

      const result = await sdk.spatial.prove(lat, lng, accuracy, confidence, sensorReadings, deviceId);
  ...
  describe('FraudDetector', () => {
    it('should flag prompt injection', async () => {
      ...
      try {
        await sdk.memory.retain(maliciousContent, {
          source: 'observation',
          sessionId: 's1',
          tags: [],
          importance: 0.5
